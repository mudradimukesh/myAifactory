...
The runner closes each worker's stdin after writing its prompt, including when the prompt is empty or omitted. Each worker segment must report its provider start event within the configured startup allowance. Codex reports `thread.started`; Claude reports `system/init`. The default startup allowance is 90 seconds. After startup, a segment may run for at most 600 seconds without new captured output or Codex rollout bytes, unless the project profile sets a different finite idle allowance. A single tool command that runs longer than 10 minutes without output is killed. Paused time consumes neither allowance. Token, context, attempt and run limits remain in force.

A missing start event ends the segment with `stall_start`. An idle expiry ends it with `stall_idle`. The runner stops the owned process group, preserves partial work and captured evidence, and records the reason on the segment, attempt result and history event. The meter records the last observed activity even when usage is unknown. A stall during a hand-off ends the logical attempt without starting another worker. The dashboard shows activity age and the recorded stall reason, with a next action to inspect the blockage and reconcile usage before retrying. Missing usage remains unknown and cannot authorize further dispatch.
...
| Worker reads non-TTY stdin until EOF | Omitted, empty and supplied prompts all end stdin; the worker does not wait for further input |
| Worker emits no provider start event | Startup allowance expires with `stall_start`; owned group stops and the reason persists even without usage or a rollout |
| Started worker produces no new capture or rollout bytes | Idle allowance expires with `stall_idle`; owned group stops and the last observed activity remains recorded |
| Worker produces activity without token usage | Capture or rollout growth postpones idle expiry; usage remains unknown |
| Context hand-off worker stalls | Logical attempt ends with its stall reason; no replacement worker starts and spent allowances remain charged |
| Dashboard observes running and stalled segments | Running row shows activity age; stalled row shows its reason and a concrete recovery action; unknown usage stays unknown |
