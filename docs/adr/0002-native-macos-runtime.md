...
Validate dedicated subscription credential homes, then copy the entire permitted provider credential file into a fresh attempt-local provider home. This permits the native CLI to refresh credentials locally. It does not extract tokens, copy ordinary user settings/plugins, mount the user home, or fall back to API keys. Providers rotate refresh tokens: live Claude runs showed that a later attempt started from the original file fails with an expired OAuth session. After an attempt, the runtime therefore writes the attempt's refreshed credential back to the source home, but only if the refreshed file still validates as a subscription credential and the source still matches the digest taken at copy time. Concurrent attempts on one provider can still invalidate each other's refresh tokens. Workers can read the credential they use. Credential files and attempt homes must remain private and must not enter evidence, proposals or handoff artifacts.
