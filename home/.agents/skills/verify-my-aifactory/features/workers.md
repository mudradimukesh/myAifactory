...
Command construction tests do not establish installed CLI compatibility or account access. Codex currently grants workspace writes to developer/tester; Claude grants Edit/Write to discovery and architecture roles as well, without Bash. Discovery and architecture scratch writes, dedicated authentication homes, and actual provider usage require separate live verification. No API billing fallback is authorized.
