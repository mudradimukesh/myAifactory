...
import { z } from 'zod';
import { stateSchema, transitions, id as idSchema } from './contracts.ts';
import type { State, Status, FileRecord, Event } from './contracts.ts';
...
export class Store {
    ...
    async lock<T>(name: string, fn: () => Promise<T>): Promise<T> {
        ...
        const release = await lockfile.lock(target, { realpath: false, stale: 10000, update: 2000, retries: 0, onCompromised: e => { compromised = e; } });
        try {
            const r = await fn();
            if (compromised)
                throw compromised;
    ...
    async create(s: State) { stateSchema.parse(s); return this.lock('run-' + s.id, async () => { const d = this.dir(s.id); if (await exists(path.join(d, 'state.json')))
        throw Error('Run already exists'); await mkdir(d, { recursive: true, mode: 0o700 }); await durable(path.join(d, 'state.json'), json(s)); await this.reconcile(s.id, s); }); }
